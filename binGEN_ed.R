...
# in a cohort stepped wedge CRT with a exponential decay correlation structure
...
# alpha: Vector of correlations: (alpha_0, rho)
#        alpha_0 = within period correlation
#        rho = decay parameter
##############################################################################################

binGEN_ed <- function(n, m, t, delta, beta, alpha){
  
  ########################################################
  # Create block exchangeable correlation matrix.
  ########################################################

  ## auxiliary function for finding the period of i
  ...
  bxch<-function(alpha, n_i){
    ...
    n_i = c(n_i)
    n_length = length(n_i)
    cor_matrix = matrix(0, sum(n_i), sum(n_i))
    useage_m = cumsum(n_i)
    ...
    for(i in 1:sum(n_i)){
        ...
        for(j in 1:sum(n_i)){
            ...
            if(i_reg == j_reg & i != j){
                ...
                cor_matrix[i,j] = alpha_0 * (rho^(abs(i_reg - j_reg)))
            }
        }
    }
    return(cor_matrix)
