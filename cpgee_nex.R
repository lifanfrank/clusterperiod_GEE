...
# the correlation matrix is set as "nested exchangeable (NEX)"
...
cpgee_nex = function(y, X, id, n, m, maxiter, epsilon, printrange, alpadj)
{
  require(MASS)
  ...
  # INPUT
  # mu: vector of n_i marginal means
  # n: sample size (scalar) for cluster
  ...
  gammahat=function(mu,alpha,n,m){
    ...
    alpha1 = alpha[2]   # between-period ICC
    v = mu*(1-mu)
    gamma_c = NULL
    for(j in 1:(n-1)){
  ...
  #####################################################################################
  # MODULE: getalpha0, getalpha1 (getalpha0, getalpha1)
  ...
  # OUTPUT
  # alpha0, alpha1:correlaion parameters
  #####################################################################################
  ...
  getalpha1=function(alpdata1){
    s = alpdata1[,1]
    vcross = alpdata1[,2]

    d1=sum(sqrt(vcross) * s)
    d0=sum(vcross)
    return(d1/d0)
  }
  ...
  if(converge==1 & ROBFLAG==0){
    ...
    return(list(outbeta = result$outbeta, 
                outalpha = result$outalpha,
                beta=beta, alpha=alpha, 
                MB=naive, BC0=robust, BC1=varKC, BC2=varMD,
