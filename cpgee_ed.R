#####################################################################################
# cluster-period GEE/MAEE of stepped wedge cluster randomized trials 
# the correlation matrix is set as "exponential decay (ED)"

# The following variance estimators are output from the program
# (1) outbeta: estimates of marginal mean model parameters and standard errors with 
#              different finite-sample bias corrections.
# (2) outalpha: estimates of correlation parameters and standard errors with 
#             different finite-sample bias corrections.
# (3) beta: a vector of estimates for marginal mean model parameters
# (4) alpha: a vector of estimates of correlation parameters
# (5) MB: model-based estimator (only available for marginal mean parameters)
# (6) BC0: uncorrected sandwich estimator that extends Liang and Zeger, 1986
# (7) BC1: bias-corrected sandwich estimator that extends Kauermann and Carroll, 2001
# (8) BC2: bias-corrected sandwich estimator that extends Mancl and DeRouen, 2001
# (9) BC3: bias-corrected sandwich estimator that extends Fay and Graubard, 2001

# INPUT
# y: vector of cluster-period means
# X: marginal mean covariates 
# id: cluster identifier
# n: a vector of cluster "sample sizes" after cluster-period aggregation
# m: a vector of the cluster-period sizes
# maxiter: maximum number of iterations for Fisher scoring updates
# epsilon: tolerance for convergence
# printrange: print details of range violations   
# alpadj: if \code{TRUE}, performs bias adjustment for the alpha estimating equations.
# rho.init: user-specified initial value for the decay parameter

# Note that all inputs are required.
#####################################################################################

cpgee_ed = function(y, X, id, n, m, maxiter, epsilon, printrange, alpadj, rho.init = NULL)
{
  require(MASS)
  require(rootSolve)
  ...
  #####################################################################################
  # MODULE: GETROWB
  # generates a row of the -E(d(cov)/dbeta) (ie Q) matrix
  ...
  # INPUT
  # gamma: vector of s_jk between mean j and mean k for cluster i
  ...
  #####################################################################################
  # MODULE: CREATED
  # creates derivative matrix for alpha estimating equation, deta/dalpha

  # INPUT
  # mu: vector of marginal means
  # alpha: correlation parameters
  # n: sample size (scalar) for cluster i
  # m: vector of cluster-period sizes
  ...
  CREATED=function(mu,alpha,n,m){
    ...
    for(j in 1:(n-1)){
      ...
      for(k in (j+1):n){
        ...
        djk1=sqrt(v[j]*v[k])*alpha0*abs(k-j)*rho^(abs(k-j)-1)
  ...
  #####################################################################################
  # MODULE: gammahat
  # Creates vector of estimated covariances

  # INPUT
  # mu: vector of n_i marginal means
  # alpha: correlation estimates
  # n: cluster size (# of periods)
  # m: vector of cluster-period sizes
  ...
  gammahat=function(mu,alpha,n,m){
    ...
    for(j in 1:(n-1)){
      ...
      for(k in (j+1):n){
        sjk=sqrt(v[j]*v[k])*alpha0*rho^(abs(k-j))
  ...
  #####################################################################################
  # MODULE: checkalpha
  # Creates compact correlation matrix for range checks
  ...
  # y: vector of outcomes (cluster-period means)
  # X: marginal mean covariates
  # m: vector of cluster-period sizes
  ...
  # INPUT
  # y: vector of cluster-period means
  # m: vector of cluster-period sizes (# of trials)
  # X: marginal mean covariates
  ...
  # INPUT
  # alpdata0: dataset built for estimating alpha0
  # alpdata1: dataset built for estimating the rest

  # OUTPUT
  # alpha0: correlation parameter
  # alpha1: correlation decay parameter
  ...
  getalpha=function(alpdata0,alpdata1,alphaold){
    ...
    f=function(alpha){
      alpha0=alpha[1]
      rho=alpha[2]
      f0_smt1=((m-1)/m)*v*(s0-v/m-alpha0*v*(m-1)/m)
      f0_smt2=sqrt(vcross)*(rho^d)*(s1-sqrt(vcross)*alpha0*rho^d)
      f0=sum(f0_smt1)+sum(f0_smt2)
      f1=sum(sqrt(vcross)*d*rho^(d-1)*(s1-sqrt(vcross)*alpha0*rho^d))
      return(c(f0,f1))
    }
  ...
  #####################################################################################
  # MODULE: INITALPHA
  # generates initial values for alpha

  # INPUT
  # y: vector of cluster-period means
  # m: vector of cluster-period sizes (# of trials)
  # X: marginal mean covariates
  # n: vector of cluster sizes (# of periods)
  # beta: estimated regression coefficients

  # OUTPUT
  # alpha: estimated correlation values
  #####################################################################################
  ...
  MAKEVAR=function(Ustarold, beta, alpha, y, X, m, n, p, q, ROBFLAG, NPSDFLAG, NPSDADJFLAG)
  {
    ...
    for(i in 1:length(n)){
      ...
      for(j in 1:n[i]){
        for(k in j:n[i]){
          if((L_c[j,k] >= min(sqrt((mu_c[j]*(1-mu_c[k]))/(mu_c[k]*(1-mu_c[j]))),
                              sqrt((mu_c[k]*(1-mu_c[j]))/(mu_c[j]*(1-mu_c[k]))))) |
             (L_c[j,k] <= max(-sqrt((mu_c[j]*mu_c[k])/((1-mu_c[j])*(1-mu_c[k]))),
    ...
    return(list(robust=robust,naive=naive,varMD=varMD,varKC=varKC,varFG=varFG,
                rangeflag=rangeflag,ROBFLAG=ROBFLAG,NPSDFLAG=NPSDFLAG,
                NPSDADJFLAG=NPSDADJFLAG))
  }

  #####################################################################################
  # MODULE: FITPRENTICE
  # Performs the paired estimating equations method
  ...
  FITPRENTICE=function(y, X, m, n, maxiter, epsilon, SINGFLAG, ROBFLAG,
    ...
    return(list(beta=beta,alpha=alpha,robust=robust,naive=naive,varMD=varMD,
                ...
                SINGFLAG=SINGFLAG,ROBFLAG=ROBFLAG,
  ...
  RESULTS=function(beta, alpha, robust, naive, varMD, varKC, varFG, niter, n){
    ...
    cat("GEE and MAEE for Cluster-Period Summaries","\n",
        ...
        "Maximum Cluster-Period Size:",max(m),"\n",
        "Minimum Cluster-Period Size:",min(m),"\n",
        "Number of Iterations:",niter,"\n",
        "Correlation Structure:",corstr,"\n")
  ...
  # Final Results
  if(SINGFLAG==1){print("Derivative matrix for beta is singular during updates")}
  if(ROBFLAG==1){print("Sandwich variance is not positive definite")}
  if(converge==0 & SINGFLAG==0){
    print("The algorithm did not converge")}
  if(converge==1 & ROBFLAG==0){
    result =  RESULTS(beta, alpha, robust, naive, varMD, varKC, varFG, niter, n)
    return(list(outbeta = result$outbeta, outalpha = result$outalpha,
